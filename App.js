...
module.controller('RootCtrl', function($scope, $log, insightsApi, projectLoader, variationCalculator){
	...
	var createHighchartsConfig = function(seriesName, series, type){
		return {
			options: {
				chart: {
		...
		if($scope.metrics && $scope.context.scorecard){
			...
			variationCalculator.calculateVariationsForProjects($scope.projects, $scope.context.dimension).then(function(variations){
				...
				_.each(projects, function(project){
					project.differenceChartConfig = createHighchartsConfig('Workspace Delta', project.variation.differences, 'column');
					project.scoresChartConfig = createHighchartsConfig('Project Scores', project.variation.projectScores, 'line');
				});
			})
		}
	}
